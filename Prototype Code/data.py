...
def LoadData(DATASET_NAME):
    """
        ...
        ; dataset object
    """
    if DATASET_NAME == 'PhysicalPlanCardinality':
        return PhysicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'PhysicalPlanCost':
        return PhysicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'PhysicalPlanTime':
        return PhysicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'LogicalPlanUndirected':
        return LogicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'LogicalPlanBigGraphs':
        return LogicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'LogicalPlanUnnormalized':
        return LogicalPlanDataset(DATASET_NAME)

    if DATASET_NAME == 'LogicalPlan':
        return LogicalPlanDataset(DATASET_NAME)
